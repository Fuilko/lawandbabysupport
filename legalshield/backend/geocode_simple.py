"""Simple prefecture-center geocoding as fallback for jageocoder.

Uses known lat/lon for each of 47 prefectures (from dataofjapan/land GeoJSON).
Processes seed CSVs and adds lat/lon columns.
"""
...
def geocode_seed(csv_path: Path, out_path: Path, pref_col: str = "prefecture") -> int:
    ...
    with open(csv_path, encoding="utf-8-sig") as f:
        ...
        for row in rdr:
            pref = row.get(pref_col) or row.get("都道府県") or ""
            lat, lon = PREF_COORDS.get(pref, (None, None))
            row["lat"] = lat
            row["lon"] = lon
            rows.append(row)
...
def main() -> None:
    seeds = [
        ("ngo_seed.csv", "ngo_seed_geocoded.csv", "prefecture"),
        ("bar_associations.csv", "bar_associations_geocoded.csv", "prefecture"),
        ("support_centers_seed.csv", "support_centers_geocoded.csv", "prefecture"),
    ]
    total = 0
    for inp, out, col in seeds:
        total += geocode_seed(
            ROOT / "knowledge" / "seeds" / inp,
            ROOT / "knowledge" / "seeds" / out,
            col,
        )
